...
if (cluster.isMaster) {
    ...
    try {
      switch (type) {
        ...
        case 'UPDATE_POLL': {
          ...
          const expiryTime = 300 + parseInt(req.body.validity || data.validity || '60');
  ...
  async function finalizePoll(pollId) {
    try {
      ...
      await redis.set(`Poll:${pollId}:final_leaderboard`, JSON.stringify(leaderboard), 'EX', 3900); // 1hr + buffer
